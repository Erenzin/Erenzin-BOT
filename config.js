	...
	const isOwner = isOwn.includes(ownerNumber)
        ...
        const mess = {
            ...
            error: {
				...
				Ga: 'Apenas Administradores podem usar, então trate de virar um seu membro comum!',
				...
				Ba: 'Caro administrador, se quiser que eu use esses comandos, precisa me deixar ser um adm!',
        ...
        await kill.reply(from, 'mn espera 10 segundos antes de usar outros comandos!', id)
        ...
        case 'stickernobg':
			if (isMedia) {
                try {
                    ...
                    console.log(err)
					await kill.reply(from, 'vish! alguma coisa deu errado nesse comando!', id)
                }
            }
            break
            ...
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 15 || mimetype === 'image/gif' && message.duration < 15) {
                    ...
                    await exec(`gify ${filename} ./lib/media/stickergf.gif --fps=15 --scale=256:256`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            kill.reply(from, 'Affer! A conversão obteve erros, talvez seja o tamanho do gif ou seu peso.', id)
                        })
                    })
                } else {
        ...
        case 'giphy':
            ...
            if (gark.length !== 1) return kill.reply(from, `baka fdp você esqueceu de inserir o link.`, id)
		...
		case 'onlyadms':
            ...
            if (onar[0] == 'on') {
				kill.setGroupToAdminsOnly(groupId, true).then(() => kill.sendText(from, 'Aqui está a prova de poder dos adm krl!\nO silenciador :O'))
			} else if (onar[0] == 'off') {
		...
		case 'setimage':
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				: kill.reply(from, 'Isso! Agora o grupo está de cara nova hehe!', id))
			} else {
				kill.reply(from, `Acho que você esta usando errado mn!`)
			}
			break			
		...
		case 'fb':
         ...
         case 'mp3': // eu censurei o acesso pois as apis estão offlines, e fazer isso evita que usem o comando e te de problemas
            ...
            .then(async(rest) => {
					...
					await kill.sendFileFromUrl(from, m3tu, '', `Titulo: ${m3ti}\nFormato:${m3fo}\n\nEspero que eu tenha acertado e...agora é so esperar! Mas evite novamente usar até que eu termine krl!`, id)
       ...
       case 'translate':
		...
		case 'resposta':
			...
			await kill.reply(from, 'Frase adicionada a Eren.', id)
			break
        ...
        case 'eren':
			try {
				const eren = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(6)}&lang=pt`)
				if (eren.data.success == '') {
					console.log('Request falhou, usando respostas locais...')
        ...
        case 'criador':
            kill.sendContact(from, '5596984081026@c.us')
       ...
       case 'ins': 
        ...
        case 'anime':
           ...
           `https://api.jikan.moe/v3/search/anime?q=${keyword}`
            ...
            if (!parsed) {
              await kill.sendFileFromUrl(from, errorurl2, 'error.png', '😞 É uma pena, não encontrei nenhum resultado...', id)
              console.log("Sent!")
              return null
              }
            ...
            translate(synopsis, 'pt')
             ...
             await kill.sendFileFromUrl(from, errorurl2, 'error.png', '😞 É uma pena, não encontrei nenhum resultado...')
           }
          break
...
/*        case 'loli':
            ...
            kill.sendFileFromUrl(from, loli.result, 'loli.jpeg', 'ai sua loli eita😏kk.', id)
            break*/
			
			
        case 'loli':
			const onefive = Math.floor(Math.random() * 145) + 1
			kill.sendFileFromUrl(from, `https://media.publit.io/file/Twintails/${onefive}.jpg`, 'loli.jpg', 'ai sua loli eita😏kk.', id)
            break
